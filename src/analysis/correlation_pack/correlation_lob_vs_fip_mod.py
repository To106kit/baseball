...
def cor_lob_vs_fip_fnc(a_team, a_year_idx, a_np_array):
    # チーム
    t_team = a_team
    t_year_idx = a_year_idx
    t_result_base_path = g.g_lob_vs_fip_result_base_path
    ...
    # FIP取得
    t_np_fip_list = common_fnc_pack.calc_fip_mod.calc_fip_fnc(a_year_idx, a_team, t_np_array)
    ...
    ax = fig.add_subplot(
        111, xlabel="LOB%", ylabel="FIP"
    )

    ax.set_xlim(30,100)
    ax.set_ylim(0.0,10)
    ax.scatter(t_np_lob_list, t_np_fip_list, s=40)

    for i, label in enumerate(t_player_name):
        plt.text(t_np_lob_list[i], t_np_fip_list[i], label)  # type: ignore

    plt.savefig(os.path.join(t_result_path, t_team + "_lob_vs_fip.png"), format="png", dpi=300)
    plt.close()

if __name__ == "__main__":
    import sys
    import os
    cor_lob_vs_fip_fnc(sys.argv[1],sys.argv[2],sys.argv[3])
