...
# WHIPを計算
def calc_whip_fnc(a_np_array):
    # 与四球取得
    t_np_basesonballs = common_fnc_pack.get_pitcher_data_mod.get_basesonballs_fnc(a_np_array)

    # 被安打数取得
    t_np_hitsallowed = common_fnc_pack.get_pitcher_data_mod.get_hitsallowed_fnc(a_np_array)
    ...
    # 【計算方法】 WHIP ＝ (被安打 ＋ 与四球) ÷ 投球回
    t_whip = np.round((t_np_hitsallowed + t_np_basesonballs) / t_np_totalOut * 3, 3)
    return t_whip